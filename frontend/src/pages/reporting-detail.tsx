...
import { TableFourFacilitiesGrid } from '../components/table-four-facilities-grid'
import { TableFiveVisitsGrid } from '../components/table-five-visits-grid'
import { TableSixEmergencyGrid } from '../components/table-six-emergency-grid'
...
export function ReportingDetailPage() {
  ...
  const [denominatorsDirty, setDenominatorsDirty] = useState(false)
  ...
  const hasUnsavedChanges = formDirty || worksheetBusy || denominatorsDirty
  const blocker = useBlocker(hasUnsavedChanges)

  useEffect(() => { if (data) { setDetail(data); setFormDirty(false); setDirtyRows([]); setDenominatorsDirty(false) } }, [data])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const method = denominatorsDirty ? 'Nilai Dasar penyebut Provinsi' : formDirty ? 'Input Ringkas' : 'Tabel Lengkap'
    if (window.confirm(`Perubahan ${method} belum tersimpan. Tinggalkan halaman dan abaikan perubahan?`)) {
      setFormDirty(false)
      setDenominatorsDirty(false)
      setDirtyRows([])
      blocker.proceed()
    } else blocker.reset()
  }, [blocker, denominatorsDirty, formDirty])
  ...
  const requiredRows = detail.rows.filter((row) => row.kind === 'base' && (['T04', 'T05', 'T06'].includes(detail.group) || row.required))
  const numericTable = ['T01', 'T02', 'T03', 'T04', 'T05', 'T06'].includes(detail.group)
  ...
  function updateDenominator(key: 'outpatient_l' | 'outpatient_p' | 'inpatient_l' | 'inpatient_p', value: string) {
    setDetail((current) => current?.provinceDenominators
      ? { ...current, provinceDenominators: { ...current.provinceDenominators, [key]: value } }
      : current)
    setDenominatorsDirty(true)
    setNotice(null)
  }
  ...
  async function save() {
    ...
    setSaving(true)
    try {
      if (isAdmin && detail.group === 'T05' && detail.scope === 'province') {
        setDetail(await api.saveTableFiveProvinceDenominators(session!.token, detail))
        setDenominatorsDirty(false)
        setNotice({ tone: 'success', message: 'Nilai Dasar penyebut Provinsi tersimpan.' })
        return
      }
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-col justify-between gap-4 border-b border-line-soft pb-4 xl:flex-row xl:items-end">
        ...
        {isAdmin && regions?.length ? <label className="flex min-w-52 flex-col gap-1 text-[11px] font-semibold text-ink-muted">{['T02', 'T03', 'T04', 'T05', 'T06'].includes(detail.group) ? 'Wilayah' : 'Kabupaten/Kota'}<Select value={detail.regionId} disabled={changingStatus || statusDialog !== null} onChange={(event) => setSearchParams((current) => { const next = new URLSearchParams(current); if (event.target.value) next.set('regionId', event.target.value); else next.delete('regionId'); next.set('year', String(year)); return next })}>{['T02', 'T03', 'T04', 'T05', 'T06'].includes(detail.group) ? <option value="">Provinsi</option> : null}{regions.map((region) => <option key={region.id} value={region.id}>{region.name}</option>)}</Select></label> : null}
        <div className="flex flex-wrap gap-2">
          ...
          {isAdmin && detail.group === 'T05' && detail.scope === 'province' ? <Button variant="outline" onClick={save} disabled={saving || !denominatorsDirty}><Save data-icon="inline-start" />{saving ? 'Menyimpan…' : 'Simpan penyebut'}</Button> : null}
      ...
      {notice?.tone === 'error' ? <FeedbackBanner className="mb-4" tone="error" title="Tindakan belum berhasil" role="alert">{notice.message}</FeedbackBanner>
        : formDirty || denominatorsDirty ? <FeedbackBanner className="mb-4" tone="warning" title="Perubahan belum disimpan" role="status">Gunakan tombol simpan di atas sebelum meninggalkan halaman.</FeedbackBanner>
      ...
      {spreadsheetView ? <TableOneWorksheetView token={session!.token} reportingTableId={detail.reportingTableId} onSynced={syncWorksheet} onBusyChange={setWorksheetBusy} /> : <Panel className="overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line bg-paper-inset px-4 py-3 text-xs"><span className="font-semibold text-ink-muted">{detail.group === 'T06' && detail.scope === 'province' ? 'Nilai Dasar tersedia sementara' : detail.scope === 'province' ? 'Data lengkap seluruh wilayah' : 'Nilai wajib'} <strong className="ml-1 font-mono text-base text-ink">{requiredRows.length - missing}/{requiredRows.length}</strong></span><span className={detail.group === 'T06' && detail.scope === 'province' ? detail.provinceCompletedRegionCount === detail.provinceRegionCount ? 'font-semibold text-archive' : 'font-semibold text-ink-muted' : missing ? 'font-semibold text-correction' : 'font-semibold text-archive'}>{detail.group === 'T06' && detail.scope === 'province' ? `${detail.provinceCompletedRegionCount || 0}/${detail.provinceRegionCount || 7} Kabupaten/Kota Selesai Input` : missing ? detail.scope === 'province' ? `${missing} rekap belum lengkap` : `${missing} belum lengkap` : 'Lengkap'}</span></div>
        {detail.group === 'T02' && detail.mappingStatus === 'ready' && !previewSession ? <TableTwoAgeGrid rows={detail.rows} editable={editable} dirtyRows={dirtyRows} onChange={updateRow} totalLabel={detail.scope === 'province' ? 'Provinsi' : 'Kabupaten/Kota'} province={detail.scope === 'province'} /> : detail.group === 'T03' && detail.mappingStatus === 'ready' && !previewSession ? <TableThreeEducationGrid rows={detail.rows} editable={editable} dirtyRows={dirtyRows} province={detail.scope === 'province'} regionName={detail.region} onChange={updateRow} /> : detail.group === 'T04' && detail.mappingStatus === 'ready' && !previewSession ? <TableFourFacilitiesGrid rows={detail.rows} editable={editable} dirtyRows={dirtyRows} province={detail.scope === 'province'} regionName={detail.region} onChange={updateRow} /> : detail.group === 'T05' && detail.mappingStatus === 'ready' && !previewSession ? <TableFiveVisitsGrid rows={detail.rows} editable={editable} dirtyRows={dirtyRows} province={detail.scope === 'province'} regionName={detail.region} onChange={updateRow} denominators={detail.provinceDenominators} coverageValues={detail.coverageValues} denominatorsDirty={denominatorsDirty} onDenominatorChange={isAdmin && detail.scope === 'province' ? updateDenominator : undefined} /> : detail.group === 'T06' && detail.mappingStatus === 'ready' && !previewSession ? <TableSixEmergencyGrid rows={detail.rows} editable={editable} dirtyRows={dirtyRows} province={detail.scope === 'province'} regionName={detail.region} onChange={updateRow} /> : <div className="overflow-x-auto scrollbar-thin"><table className="w-full min-w-[680px] border-collapse text-left text-xs">
