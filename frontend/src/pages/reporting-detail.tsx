    ...
    <>
      ...
      <div className="mb-5 flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
        <div className="min-w-0">
          <div className="mb-2 flex flex-wrap items-center gap-2"><span className="font-mono text-xs font-bold text-archive">TABEL {String(detail.number).padStart(2, '0')}</span><StatusBadge status={detail.status} />{detail.status === 'verified' ? <span className="inline-flex items-center gap-1 text-[11px] text-ink-muted"><LockKeyhole className="size-3" />Terkunci</span> : null}</div>
          <h1 className="text-xl font-bold tracking-[-0.025em] sm:text-2xl">{detail.name}</h1>
          <p className="mt-1 max-w-3xl text-sm text-ink-muted">{detail.description}</p>
