...
export function ReportingDetailPage() {
  ...
  const requiredRows = detail.rows.filter((row) => row.kind === 'base' && (['T04', 'T05'].includes(detail.group) || row.required))
  const numericTable = ['T01', 'T02', 'T03', 'T04', 'T05'].includes(detail.group)
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-col justify-between gap-4 border-b border-line-soft pb-4 xl:flex-row xl:items-end">
        ...
        {isAdmin && regions?.length ? <label className="flex min-w-52 flex-col gap-1 text-[11px] font-semibold text-ink-muted">{['T02', 'T03', 'T04', 'T05'].includes(detail.group) ? 'Wilayah' : 'Kabupaten/Kota'}<Select value={detail.regionId} disabled={changingStatus || statusDialog !== null} onChange={(event) => setSearchParams((current) => { const next = new URLSearchParams(current); if (event.target.value) next.set('regionId', event.target.value); else next.delete('regionId'); next.set('year', String(year)); return next })}>{['T02', 'T03', 'T04', 'T05'].includes(detail.group) ? <option value="">Provinsi</option> : null}{regions.map((region) => <option key={region.id} value={region.id}>{region.name}</option>)}</Select></label> : null}
