...
class WorkbookCatalogImporterTest extends TestCase
{
    ...
    public function test_reimport_preserves_ready_table_two_to_six_mappings(): void
    {
        $year = ReportingYear::create(['year' => 2024, 'status' => 'open']);
        foreach (['T02', 'T03', 'T04', 'T05', 'T06'] as $code) {
        ...
        $sheetSix = $workbook->createSheet();
        $sheetSix->setTitle('6');
        $sheetSix->setCellValue('A3', 'TABEL 6 UJI');
        (new Xlsx($workbook))->save($path);
        ...
        try {
            ...
            $this->assertDatabaseHas('reporting_tables', ['reporting_year_id' => $year->id, 'code' => 'T06', 'mapping_status' => 'ready']);
        } finally {
